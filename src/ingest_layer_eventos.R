...
eventos_dgoat_geoambiental %>%
  st_write("data/interm/eventos_ocurrencia/eventos_dgoat_geoambiental.gpkg")
...
municipios_car <- read_sf("data/interm/base/municipios.gpkg")
municipios_filter <- municipios_car$CODDANE %>% unique()
...
eventos_ungrd_datos_abiertos %>%
  write_csv("data/interm/eventos_ocurrencia/eventos_ungrd_datosabiertos.csv")
...
municipios_filter <- municipios_car$Municipio %>% unique() %>%
...
eventos_ungrd_gestiondelriesgo %>%
  write_csv("data/interm/eventos_ocurrencia/eventos_ungrd_gestiondelriesgo.csv")
...
writeRaster(burn_rasters_car, filename = "data/interm/satelital/modis_areas_quemadas.tif")
...
municipios_to_join <- municipios_car %>% select(Municipio, CODDANE) %>%
...
modis_points %>%
  st_write("data/interm/eventos_ocurrencia/eventos_modis_mcd64a1.gpkg")
...
left_join(
  eventos_dgoat_geoambiental %>% tibble() %>% 
  ...
  eventos_ungdr_gestiondelriesgo %>% tibble() %>% 
    mutate(Municipio = stri_trans_general(municipio, "Latin-ASCII") %>% toupper()) %>%
...
ggplot(df_events_year, aes(x = factor(year), y = total_events, fill = metodo)) +
...
# 2. Gráfico de barras agrupadas
ggplot(df_events_year2, aes(x = factor(year), y = total_events, fill = metodo)) +
  geom_col(position = position_dodge()) +
  scale_y_continuous(labels = label_comma()) +
  labs(
    title = "Número de eventos por fuente y año",
