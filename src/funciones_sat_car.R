...
crear_paleta_clc <- function(nivel_clc) {
  ...
  nivel_3 <- nivel_3 %>%
    ...
    dplyr::select(codigo_simple, leyenda, nivel, color)
...
plot_raster_clc <- function(raster_clc, clc_level = 2, tag = "") {
  ...
  grupos <- crear_paleta_clc(nivel_clc)  %>%
    dplyr::mutate(color = ifelse(is.na(color), "#808080", color)) %>%
    dplyr::arrange(leyenda)
...
#calcula area raster
calcula_area_clc <- function(test_raster, pixel_resol = 250){
  
  # Calcular la frecuencia de cada valor único
  frecuencias <- freq(test_raster, digits = 0)
  
  # Agregar el área total de píxeles por valor
  # Área de un píxel (en m²)
  area_pixel <- pixel_resol^2
  
  # Agregar columna con el área total
  frecuencias$area_total <- frecuencias$count * area_pixel /10000
